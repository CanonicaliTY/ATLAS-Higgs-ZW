#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# v1.2 update: added support for the "tight parquet"
...
SETTINGS = {
    ...
    # Data loading
    # When USE_TIGHT_PARQUET=False this is the fraction of the ORIGINAL ATLAS parquet to read.
    # When USE_TIGHT_PARQUET=True this is the fraction of the TIGHT parquet to read back in.
    "FRACTION": 1.0,
    ...
    "ISO_SCAN": {
        ...
        # Can be a single float, or a dict like {"mu": 0.995, "e": 0.98}
        "OS_SIG_EFF_MIN": {"mu": 0.995, "e": 0.98},
    },

    # Tight parquet support (Lab script 7.5 idea)
    # Use write_parquet=True to build smaller OS / SS parquet files once, then re-read them safely with FRACTION=1.
    "USE_TIGHT_PARQUET": True,
    "TIGHT_PARQUET": {
        # Build from the ORIGINAL ATLAS parquet with this fraction
        # (usually 1.0; write_parquet + return_output=False keeps memory usage low)
        "BUILD_FRACTION": 1.0,
        ...
        # If True, delete and rebuild the tight parquet even if it already exists
        "FORCE_REBUILD": False,

        # Preselection stage to save:
        #   "baseline" : 2 leptons + flavour + trigger + pT + OS/SS split + mass computed
        #   "mass"     : baseline + mass window
        ...
        # Write a small metadata JSON next to the generated parquet
        "WRITE_METADATA": True,
    },
...
def iso_eff_threshold_for(lepton: str) -> float:
    """
    Return the OS signal-efficiency threshold used by the isolation scan.
    Accepts either:
      - a single float in SETTINGS["ISO_SCAN"]["OS_SIG_EFF_MIN"], or
      - a dict like {"mu": 0.995, "e": 0.98}.
    """
    thr = SETTINGS["ISO_SCAN"]["OS_SIG_EFF_MIN"]
    if isinstance(thr, dict):
        return float(thr[lepton])
    return float(thr)


def _tight_stage_kwargs(stage: str,
                        mass_window: tuple[float, float],
                        ptcone_max: float,
                        etcone_max: float) -> dict:
    stage = stage.lower().strip()
    if stage == "baseline":
        return {}
    if stage == "mass":
        return {"mass_window": mass_window}
    if stage == "mass_iso":
        return {"mass_window": mass_window, "ptcone_max": ptcone_max, "etcone_max": etcone_max}
    raise ValueError("TIGHT_PARQUET['STAGE'] must be one of: 'baseline', 'mass', 'mass_iso'")


def _slim_for_tight(data: ak.Array, keep_fields: list[str]) -> ak.Array:
    """
    ...
    for wname in ("weight", "totalWeight"):
        if (wname in data.fields) and (wname not in keep):
            keep.append(wname)

    if len(keep) == 0:
        return data

    # Rebuild a record array with only the requested fields.
...
def make_tight_cut_function(lepton: str,
    ...
    def _cut(data: ak.Array) -> ak.Array:
        ...
        d = apply_selection(d, require_both=require_both, **stage_kwargs)
        if len(d) == 0:
            return d

        return _slim_for_tight(d, keep_fields)

    return _cut


def tight_tag(lepton: str) -> str:
    """
    Auto-generate a readable directory tag for the tight parquet.
    Example:
        mu_baseline_pt10p0
        e_mass_pt10p0_m66p0_116p0
    """
    user_tag = SETTINGS["TIGHT_PARQUET"]["TAG"]
    if user_tag:
        return str(user_tag)

    stage = SETTINGS["TIGHT_PARQUET"]["STAGE"]
    pt_min = str(SETTINGS["PT_MIN"]).replace(".", "p")
    lo, hi = SETTINGS["MASS_WINDOW"]
    tag = f"{lepton}_{stage}_pt{pt_min}"
    ...
    if stage == "mass_iso" and not SETTINGS["USE_SCAN"]:
        ...
        tag += f"_ptc{str(ptc).replace('.', 'p')}_etc{str(etc).replace('.', 'p')}"
        tag += "_both" if SETTINGS["REQUIRE_BOTH_ISO"] else "_lead"

    return tag


def tight_root(lepton: str, sign: str) -> Path:
    root_dir = Path(SETTINGS["TIGHT_PARQUET"]["ROOT_DIR"]).resolve()
    return root_dir / tight_tag(lepton) / sign.upper()


def tight_ready(root: Path, samples: list[str]) -> bool:
    """
    Check whether a tight parquet directory already exists and looks complete enough to reuse.
    """
    if not root.exists():
        return False
    for sample in samples:
        ...
        if not sdir.exists():
            return False
        if not list(sdir.glob("chunk_*.parquet")):
            return False
    return True
...
def available_parquet_fields(read_directory: Path) -> list[str]:
    """
    Inspect the first chunk in *each* sample subdirectory and return the union of column names.

    Why union instead of "first file only"?
    Because data and MC do not always have identical fields: the MC weight field may be absent
    in the data subdirectory but present in the MC subdirectories.
    """
    files = sorted(read_directory.glob("*/*.parquet"))
    if len(files) == 0:
        raise FileNotFoundError(f"No parquet files found under {read_directory}")

    fields = set()
    for f in files:
        try:
            fields.update(pq.ParquetFile(f).schema.names)
        except Exception:
            continue
...
def ensure_tight_parquet(lepton: str, sign: str, cfg: dict, backend: dict) -> Path:
    """
    Build (or reuse) a tighter parquet directory for one lepton channel and one sign (OS / SS).
    This is the core 7.5-style memory-saving step.
    """
    ...
    stage = SETTINGS["TIGHT_PARQUET"]["STAGE"]
    if (stage == "mass_iso") and SETTINGS["USE_SCAN"]:
        raise RuntimeError(
            "TIGHT_PARQUET['STAGE']='mass_iso' requires USE_SCAN=False, "
            "because the final isolation cut is not known before the scan."
        )
...
def run_channel(lepton: str, backend: dict, outdir: Path) -> None:
    ...
    if SETTINGS["USE_TIGHT_PARQUET"]:
        os_root = ensure_tight_parquet(lepton, "OS", cfg, backend)
        ss_root = ensure_tight_parquet(lepton, "SS", cfg, backend)

        read_vars_os = read_vars_for_tight_directory(os_root)
        read_vars_ss = read_vars_for_tight_directory(ss_root)

        print(f"[{lepton}] reading tight parquet (OS) with vars: {read_vars_os}")
        data_OS = analysis_parquet(
            read_vars_os,
            string_code_list=None,
            read_directory=str(os_root),
        ...
        print(f"[{lepton}] reading tight parquet (SS) with vars: {read_vars_ss}")
        data_SS = analysis_parquet(
            read_vars_ss,
            string_code_list=None,
            read_directory=str(ss_root),
            subdirectory_names=None,
            fraction=SETTINGS["FRACTION"],
            cut_function=None,
            write_parquet=False,
    ...
    with open(summary_txt, "w", encoding="utf-8") as f:
        ...
        f.write(f"Use tight parquet: {SETTINGS['USE_TIGHT_PARQUET']}\n")
        if SETTINGS["USE_TIGHT_PARQUET"]:
